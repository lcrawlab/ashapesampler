...
for (k in 1:N){
  ...
  temp_torus <- ashape3d(torus_pts, alpha_vec[k])
  torus_list[[k]] <- temp_torus
  complex_torus_list[[k]] <- get_alpha_complex(torus_pts, alpha_vec[k])
...
```{r}
point_cloud = matrix(NA, ncol = 3)
for (k in 1:N) {
  if (k == 1) {
    point_cloud = torus_list[[k]]$x
  } else{
...
```{r}
new_torus <- generate_ashape3d(point_cloud, N=N, tau=tau, bound="cube")

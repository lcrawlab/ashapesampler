...
We then load this png image using the `image_import` function from the `pliman` library and manually convert it to a binary mask. We correct for the black border that ensures centering of the image when we saved it.
...
img_binR <- t(img_bin$R)
my_ind = which(img_binR[2:(dim_square-1), 2:(dim_square-1)] < 1, arr.ind=TRUE)
img_bin = matrix(0, nrow=dim_square, ncol=dim_square)
img_bin[my_ind] = 1
image(as.matrix.csr(img_bin))
```
While not a perfect recovery of the original binary mask, the basic tutorial shows the general process for going back and forth. Preservation improves with more detailed pixelation.

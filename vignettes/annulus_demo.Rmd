...
simulating a new shape from that. This vignette requires the packages \emph{alphahull},
\emph{truncnorm}, and \emph{ggplot2} in addition to \emph{ashapesampler}.
...
for (k in 1:N){
  ...
  ann_list[[k]] <- ashape(ann_pts, alpha = alpha_vec[k])
  complex_list[[k]] <- get_alpha_complex(ann_pts, alpha_vec[k])
  edge_list <- extract_complex_edges(complex_list[[k]], n)
...
```{r}
point_cloud = matrix(NA, ncol = 2)
for (k in 1:N) {
  if (k == 1) {
    point_cloud = ann_list[[k]]$x
  } else{
...
as well.
```{r}
new_annulus <- generate_ashape2d(point_cloud, N=N, tau=tau, bound="square")

tri_keep = new_annulus$delvor.obj$tri.obj$trlist[which(new_annulus$delvor.obj$tri.obj$cclist[,3]<my_alpha), 1:3]
...
extremes = as.data.frame(points[new_annulus$alpha.extremes,])
